...
export function ProfileSettingsPanel({ profile, repo, onClose }: ProfileSettingsPanelProps) {
  ...
  const [passwordCodeLeft, setPasswordCodeLeft] = useState('');
  const [passwordCodeRight, setPasswordCodeRight] = useState('');
  ...
  const handleVerifyPasswordCode = async () => {
    const code = normalizeDigits(`${passwordCodeLeft}${passwordCodeRight}`);
    if (code.length < 6) {
      ...
      <div className="border-hair w-full max-w-[720px] overflow-hidden rounded-[10px] border bg-[#080402] p-3 shadow-[0_34px_100px_-42px_var(--glow)] sm:p-7">
        ...
        {screen === 'password-code' && (
          <OtpStep
            ...
            left={passwordCodeLeft}
            right={passwordCodeRight}
            setLeft={setPasswordCodeLeft}
            setRight={setPasswordCodeRight}
            disabled={isBusy}
...
function SingleInputStep({
  ...
  return (
    <StepShell title={title} description={description}>
      ...
      <PrimaryAction disabled={disabled} onClick={onSubmit}>
        {action}
      </PrimaryAction>
    </StepShell>
  );
  ...
  return (
    <StepShell title={title} description={description}>
      <div dir="ltr" className="flex items-center gap-3">
        <OtpBox value={left} onChange={setLeft} disabled={disabled} />
        <span className="text-ember text-xl font-black">•</span>
        <OtpBox value={right} onChange={setRight} disabled={disabled} />
      </div>
      <PrimaryAction disabled={disabled} onClick={onSubmit}>
        ادامه
      </PrimaryAction>
...
function OtpBox({
  ...
  return (
    <input
      dir="ltr"
      value={value}
      onChange={(event) => onChange(normalizeDigits(event.target.value).slice(0, 3))}
      inputMode="numeric"
      disabled={disabled}
      className="focus:border-ember h-11 w-full rounded-[6px] border border-[rgba(255,98,0,.18)] bg-[rgba(255,98,0,.12)] px-3 text-center text-lg font-black tracking-[.55em] outline-none disabled:opacity-60"
    />
  );
}
